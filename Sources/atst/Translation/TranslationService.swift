...
struct TranslationService {
    ...
    func streamTranslateText(
        _ text: String,
        onDelta: @escaping @MainActor (String) -> Void
    ) async throws -> String {
        let model = configuration.textModel.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !model.isEmpty else {
            throw AppError.noTextModelConfigured
        }

        let request = buildPromptRequest(for: text)
        AppLogger.log("translation prompt phonetic=\(request.includePhonetic) explanation=\(request.includeDescription) text='\(text.prefix(60))'")
    ...
    private func buildPromptRequest(for text: String) -> PromptRequest {
        let isLookup = TranslationService.looksLikeLookupTerm(text)
        let includePhonetic = configuration.phoneticEnabled && isLookup
        let includeDescription = configuration.smartExplanationEnabled && (isLookup || TranslationService.canEnrich(text))
