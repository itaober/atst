...
- ⌨️ **手动输入翻译** — 没选中文字时按 `⌥D`，或点设置面板的键盘图标，输入 / 粘贴即可
...
3. 启动 atst — 菜单栏右上角会出现一个翻译图标
4. 首次启动时 atst 会请求 **辅助功能** 权限——没有它全局快捷键无法工作。点 **打开系统设置**，在 隐私与安全性 → 辅助功能 里打开 atst。错过了对话框？点菜单栏图标，用权限行右侧的齿轮按钮
...
- **快捷键没反应** — 辅助功能未授权（点菜单栏图标，权限行会是橙点），或者别的 App 开了 macOS 安全键盘输入（1Password 自动填充、Terminal 的 Secure Keyboard Entry、密码输入框获得焦点）；这种情况下设置面板会显示警告。
